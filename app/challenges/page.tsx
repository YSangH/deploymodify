  ...
  const createChallenge = async (e: React.FormEvent) => {
    ...
    const requestData: AddChallengeRequestDto = {
      ...
      startTime: formData.startTime
        ...
        : null,
      endTime: formData.endTime ? new Date(`2000-01-01T${formData.endTime}`).toISOString() : null,
    };
  ...
  const renderChallengeCard = (challenge: ChallengeDto) => (
    ...
    >
      ...
      <div className='space-y-2 text-sm text-gray-600'>
        <div>
          <span className='font-medium'>기간:</span> {formatDate(challenge.createdAt)} ~{' '}
          {formatDate(challenge.endAt)}
        </div>

        <div>
          <span className='font-medium'>시간:</span> {formatTime(challenge.startTime)} ~{' '}
        ...
        <div>
          <span className='font-medium'>카테고리 ID:</span> {challenge.categoryId}
        </div>

        <div>
          <span className='font-medium'>사용자 ID:</span>
          <span className='text-xs font-mono bg-gray-100 px-1 rounded'>{challenge.userId}</span>
        </div>
      </div>
  ...
  return (
    <div className='container mx-auto p-6'>
      ...
      <div className='bg-white border rounded-lg p-6 mb-8 shadow-sm'>
        ...
        <form onSubmit={createChallenge} className='space-y-4'>
          <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
            <div>
              <label className='block text-sm font-medium text-gray-700 mb-1'>챌린지 이름 *</label>
              <input
                type='text'
                name='name'
                value={formData.name}
          ...
          {createChallengeMutation.isError && (
            <div className='bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded'>
              <strong>오류:</strong>{' '}
              {createChallengeMutation.error?.message || '챌린지 생성에 실패했습니다.'}
            </div>
          )}
      ...
      <div className='bg-white border rounded-lg p-6 mb-8 shadow-sm'>
        ...
        <div className='mb-4'>
          <label className='block text-sm font-medium text-gray-700 mb-2'>챌린지 ID</label>
          ...
          <div className='border rounded-lg p-4 bg-yellow-50'>
            ...
            <form className='space-y-4'>
              ...
              {updateChallengeMutation.isError && (
                <div className='bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded'>
                  <strong>오류:</strong>{' '}
                  {updateChallengeMutation.error?.message || '챌린지 수정에 실패했습니다.'}
                </div>
              )}
              ...
              <div className='flex gap-4'>
                ...
                >
                  {updateChallengeMutation.isPending ? '수정중...' : '수정 완료'}
                </button>
                <button
                  type='button'
        ...
        {detailError && (
          <div className='bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded'>
            <strong>오류:</strong> {detailError.message || '챌린지 조회에 실패했습니다.'}
          </div>
        )}
        ...
        {deleteChallengeMutation.isError && (
          <div className='bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded'>
            <strong>오류:</strong>{' '}
            {deleteChallengeMutation.error?.message || '챌린지 삭제에 실패했습니다.'}
          </div>
        )}
      </div>
      ...
      <div className='bg-white border rounded-lg p-6 mb-8 shadow-sm'>
        ...
        {categoryError && (
          <div className='bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded mb-4'>
            <strong>오류:</strong> {categoryError.message || '카테고리별 조회에 실패했습니다.'}
          </div>
        )}
        ...
        {!categoryLoading && !categoryError && (
          <div>
            <p className='text-gray-600 mb-4'>
              카테고리 {selectedCategory}의 챌린지: {categoryChallenges.length}개
            </p>
      ...
      <div className='bg-white border rounded-lg p-6 shadow-sm'>
        ...
        {challengesError && (
          <div className='bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded mb-4'>
            <strong>오류:</strong> {challengesError.message || '챌린지 목록 조회에 실패했습니다.'}
          </div>
        )}
        ...
        {!loading && !challengesError && (
          <div>
            <p className='text-gray-600 mb-4'>총 {challenges.length}개의 챌린지가 있습니다.</p>

            {challenges.length === 0 ? (
              <p className='text-gray-500 text-center py-8'>등록된 챌린지가 없습니다.</p>
            ) : (
              <div className='grid gap-4 md:grid-cols-2 lg:grid-cols-3'>
