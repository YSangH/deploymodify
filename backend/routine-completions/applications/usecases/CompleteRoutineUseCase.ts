...
import { CreateRoutineCompletionRequestDto, CreateRoutineCompletionResponseDto } from "../dtos/RoutineCompletionDto";
...
export class CompleteRoutineUseCase {
  ...
  async execute(request: CreateRoutineCompletionRequestDto): Promise<CreateRoutineCompletionResponseDto> {
    ...
    const createdCompletion = await this.routineCompletionsRepository.create(routineCompletionToCreate);
