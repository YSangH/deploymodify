...
export class GetRoutineCompletionsUseCase {
  ...
  async getByUserAndRoutine(userId: string, routineId: number): Promise<CreateRoutineCompletionResponseDto[]> {
    const completions = await this.routineCompletionsRepository.findByUserIdAndRoutineId(userId, routineId);
