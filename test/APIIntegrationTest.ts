  ...
  if (testRoutineId) {
    try {
      ...
      if (response.ok) {
        ...
        console.log(`- 상태코드: ${response.status}`);
        console.log(`- 루틴 제목: ${response.data?.routineTitle}`);
        testResults.push({ test: `GET /api/routines/${testRoutineId}`, success: true, data: response.data });
      } else {
        console.log("❌ 특정 루틴 조회 실패");
        console.log(`- 상태코드: ${response.status}`);
        console.log(`- 에러: ${JSON.stringify(response.data)}`);
        testResults.push({ test: `GET /api/routines/${testRoutineId}`, success: false, error: response.data });
      }
    } catch (error) {
      console.log("❌ 특정 루틴 조회 중 예외 발생:", error);
      testResults.push({ test: `GET /api/routines/${testRoutineId}`, success: false, error: String(error) });
    }
  }
  ...
  if (testRoutineId) {
    try {
      ...
      if (response.ok) {
        ...
        testResults.push({ test: `PUT /api/routines/${testRoutineId}`, success: true, data: response.data });
      } else {
        console.log("❌ 루틴 수정 실패");
        console.log(`- 상태코드: ${response.status}`);
        console.log(`- 에러: ${JSON.stringify(response.data)}`);
        testResults.push({ test: `PUT /api/routines/${testRoutineId}`, success: false, error: response.data });
      }
    } catch (error) {
      console.log("❌ 루틴 수정 중 예외 발생:", error);
      testResults.push({ test: `PUT /api/routines/${testRoutineId}`, success: false, error: String(error) });
    }
  }
}
...
async function testRoutineCompletionsAPI() {
  ...
  try {
    ...
    const response = await makeAPIRequest('POST', `/routines/${testRoutineId}/complete`, completionData);
    
    if (response.ok) {
      console.log("✅ 루틴 완료 처리 성공");
      console.log(`- 상태코드: ${response.status}`);
      console.log(`- 완료 기록 ID: ${response.data?.id}`);
      testResults.push({ test: `POST /api/routines/${testRoutineId}/complete`, success: true, data: response.data });
      ...
      console.log(`- 상태코드: ${response.status}`);
      console.log(`- 에러: ${JSON.stringify(response.data)}`);
      testResults.push({ test: `POST /api/routines/${testRoutineId}/complete`, success: false, error: response.data });
    }
  } catch (error) {
    console.log("❌ 루틴 완료 처리 중 예외 발생:", error);
    testResults.push({ test: `POST /api/routines/${testRoutineId}/complete`, success: false, error: String(error) });
  }
  ...
  try {
    ...
    if (response.ok) {
      ...
      console.log(`- 상태코드: ${response.status}`);
      console.log(`- 완료 기록 개수: ${Array.isArray(response.data) ? response.data.length : 'N/A'}`);
      testResults.push({ test: `GET /api/routines/${testRoutineId}/complete`, success: true, data: response.data });
    } else {
      console.log("❌ 루틴 완료 목록 조회 실패");
      console.log(`- 상태코드: ${response.status}`);
      console.log(`- 에러: ${JSON.stringify(response.data)}`);
      testResults.push({ test: `GET /api/routines/${testRoutineId}/complete`, success: false, error: response.data });
    }
  } catch (error) {
    console.log("❌ 루틴 완료 목록 조회 중 예외 발생:", error);
    testResults.push({ test: `GET /api/routines/${testRoutineId}/complete`, success: false, error: String(error) });
  }

  // 3. 특정 사용자의 완료 기록 조회
  try {
    console.log(`\n3. 특정 사용자의 루틴 완료 기록 조회 테스트`);
    const response = await makeAPIRequest('GET', `/routines/${testRoutineId}/complete?userId=test-api-user-12345`);
    
    if (response.ok) {
      console.log("✅ 특정 사용자의 완료 기록 조회 성공");
      console.log(`- 상태코드: ${response.status}`);
      console.log(`- 완료 기록 개수: ${Array.isArray(response.data) ? response.data.length : 'N/A'}`);
      testResults.push({ test: `GET /api/routines/${testRoutineId}/complete?userId=test-api-user-12345`, success: true, data: response.data });
    } else {
      console.log("❌ 특정 사용자의 완료 기록 조회 실패");
      console.log(`- 상태코드: ${response.status}`);
      console.log(`- 에러: ${JSON.stringify(response.data)}`);
      testResults.push({ test: `GET /api/routines/${testRoutineId}/complete?userId=test-api-user-12345`, success: false, error: response.data });
    }
  } catch (error) {
    console.log("❌ 특정 사용자의 완료 기록 조회 중 예외 발생:", error);
    testResults.push({ test: `GET /api/routines/${testRoutineId}/complete?userId=test-api-user-12345`, success: false, error: String(error) });
  }
}
