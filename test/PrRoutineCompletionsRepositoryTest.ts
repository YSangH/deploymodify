...
async function testFindByUserIdAndRoutineId() {
  ...
  try {
    ...
    const userId = 'test-user-id-12345';
    const routineId = 1;
    const userRoutineCompletions = await routineCompletionsRepository.findByUserIdAndRoutineId(
      userId,
      routineId
    );

    console.log(
      `사용자 ${userId}의 루틴 ${routineId} 완료 기록 개수:`,
      userRoutineCompletions.length
    );
...
async function testUpdate() {
  ...
  try {
    ...
    // 루틴 완료 업데이트
    const updatedCompletion = await routineCompletionsRepository.update(
      createdCompletionId,
      updateData
    );
