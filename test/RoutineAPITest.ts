...
async function makeRequest(method: string, endpoint: string, data?: any): Promise<TestResult> {
  try {
    ...
    if (result.success) {
      ...
      if (parsedResponse) {
        console.log(`📋 응답:`, typeof parsedResponse === 'string' 
          ? parsedResponse 
...
function analyzeTestResults() {
  ...
  console.log("\n📈 HTTP 상태코드 분포:");
  Array.from(statusCodes.entries()).sort().forEach(([code, count]) => {
  ...
  testResults.forEach((result, index) => {
    ...
    console.log(`${(index + 1).toString().padStart(2)}. ${status} ${result.method} ${result.endpoint}${statusCode}`);
